#pragma once
#include <cstdint>
#include <cstring>

static unsigned char decrBuffer[4096];

static unsigned char sampDecrTable[256] = {
    
};

static const uint8_t sampEncrTable[256] = {
    
};

inline bool sampDecrypt(uint8_t* buf, int len, int port, int unk) {
    if (len < 2) return false;
    unsigned char bChecksumEncr = buf[0];
    len--;
    memcpy(decrBuffer, buf + 1, len);
    unsigned char bPort = (unsigned char)(port ^ 0xCC);
    for (int i = 0; i < len; i++) {
        if (!unk) { decrBuffer[i] = (unsigned char)(unk ^ decrBuffer[i]); unk++; }
        else { decrBuffer[i] = bPort ^ decrBuffer[i]; unk--; }
        decrBuffer[i] = sampDecrTable[decrBuffer[i]];
    }
    unsigned char bChecksum = 0;
    for (int i = 0; i < len; i++) bChecksum ^= (decrBuffer[i] & 0xAA);
    if (bChecksum != bChecksumEncr) {
        printf("checksum mismatch: calc=0x%02X expected=0x%02X\n", bChecksum, bChecksumEncr);
        return false;
    }
    return true;
}

inline void samp_encrypt(uint8_t* buf, int len, int port, int unk) {
    uint8_t checksum = 0;
    for (int i = 0; i < len; i++)
        checksum ^= (buf[i] & 0xAA);
    static uint8_t tmp[4096];
    const uint8_t  bPort = static_cast<uint8_t>(port ^ 0xCC);
    for (int i = 0; i < len; i++) {
        tmp[i] = sampEncrTable[buf[i]];
        if (unk)
            tmp[i] ^= bPort;
        unk ^= 1;
    }
    buf[0] = checksum;
    memcpy(buf + 1, tmp, len);
}